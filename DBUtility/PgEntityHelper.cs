...
namespace DBUtility
{
    ...
    public class PgEntityHelper
    {
        ...
        /// <summary>
        ///  将NpgsqlDataReader转换成数据实体
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="row"></param>
        /// <param name="relation"></param>
        /// <returns></returns>
        public static T GetEntityByDataReader<T>(NpgsqlDataReader reader, Hashtable relation)
        {
            Type type = typeof(T);
            T destObj = default(T);
            if (reader.HasRows)
            ...
            foreach (PropertyInfo prop in ModelPropertyInfoCache.Instance.GetProperties<T>())
            {
                try
                {
        ...
        /// <param name="relation">数据库表列名与对象属性名对应关系；如果列名与实体对象属性名相同，该参数可为空</param>
        /// <returns>对象列表</returns>
        public static List<T> GetEntityListByDataReader<T>(NpgsqlDataReader reader, Hashtable relation)
        {
            List<T> list = null;
            T destObj = default(T);
            if (reader.HasRows)
        ...
        /// <param name="relation"></param>
        /// <returns></returns>
        public static T IGetEntityByDataReader<T>(IDataReader reader, Hashtable relation)
        {
            Type type = typeof(T);
            T destObj = default(T);
        ...
        /// <param name="relation">数据库表列名与对象属性名对应关系；如果列名与实体对象属性名相同，该参数可为空</param>
        /// <returns>对象列表</returns>
        public static List<T> IGetEntityListByDataReader<T>(IDataReader reader, Hashtable relation)
        {
            List<T> list = null;
            T destObj = default(T);
            if (reader.FieldCount > 0)
            ...
            while (reader.Read())
            {
                destObj = IGetEntityByDataReader<T>(reader, relation);
                list.Add(destObj);
            }
